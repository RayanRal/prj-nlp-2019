    ...
    with open(filename, "r") as f:
        ...
        for line in text:
            if len(line) == 0:
                ...
                tokens.append(("<S>", pos, pos))  # sentence start
                for token in line.split(" "):
                    tokens.append((token, pos, pos + len(token)))
                    pos += len(token) + 1
                tokens.append(("</S>", pos, pos))  # sentence end
    return tokens
...
def get_train_test():
    dev_test = _read_dev_test_split()
    train_tokens, test_tokens, train_labels, test_labels = [], [], [], []

    for filename in dev_test["dev"]:
        try:
            tokens = read_tokens(PATH + "/raw/" + filename + ".txt")
            train_tokens += [token[0] for token in tokens]
            train_labels += extract_labels(read_annotations(PATH + "/raw/" + filename + ".ann"), tokens)
        except:
            pass
    ...
    for filename in dev_test["test"]:
        try:
            ...
            test_labels += extract_labels(read_annotations(PATH + "/raw/" + filename + ".ann"), tokens)
        except:
            pass
